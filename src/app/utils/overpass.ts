import { calculateBearing, calculateDistance, Instruction, Point } from './geo';
 ...
 */
export const fetchIntersections = async (instructions: Instruction[]): Promise<Instruction[]> => {
...
const processChunk = async (instructions: Instruction[]): Promise<{ id: number; branches: number[] }[]> => {
    ...
    return instructions.map(inst => {
        ...
        if (!centerNode) return { id: inst.id, branches: [] };
